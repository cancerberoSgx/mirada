...
export async function run<T extends ScriptOperation[]>(options: RunOptions<T>) {
  ...
  while ((op = await gen.next(context) as any)) {
    options.debug && console.log('run operation "' + JSON.stringify(op))
    ...
    const opOptions = { ...op, src: src.mat, dst: dst ? dst.mat : undefined }
    new Class().exec(opOptions as any)
    if (!dst) {
      context.images.push({ name: op.dst, mat: opOptions.dst! })
    }
  }
  return context
}
