 ...
 * 
 * [threshold](#d7/d1b/group__imgproc__misc_1gae8a4a146d1ca78c626a53577199e9c57}), [blur](#d4/d86/group__imgproc__filter_1ga8c45db9afe636703801b0b2e440fce37}), [GaussianBlur](#d4/d86/group__imgproc__filter_1gaabe8c836e97159a9193fb0b11ac52cf1})
 * 
...
/**
 * The function [cv::distanceTransform](#d7/d1b/group__imgproc__misc_1ga8a0b7fdfcb7a13dde018988ba3a43042}) calculates the approximate or precise distance from every binary image pixel to the nearest zero pixel. For zero image pixels, the distance will obviously be zero.
 * 
 * When maskSize == [DIST_MASK_PRECISE](#d7/d1b/group__imgproc__misc_1ggaaa68392323ccf7fad87570e41259b497a71e13e06c1d12eabd2acd2669f94f9ca}) and distanceType == [DIST_L2](#d7/d1b/group__imgproc__misc_1ggaa2bfbebbc5c320526897996aafa1d8ebaff0d1f5be0fc152a56a9b9716d158b96}) , the function runs the algorithm described in Felzenszwalb04 . This algorithm is parallelized with the TBB library.
 * 
 * In other cases, the algorithm Borgefors86 is used. This means that for a pixel the function finds the shortest path to the nearest zero pixel consisting of basic shifts: horizontal, vertical, diagonal, or knight's move (the latest is available for a `$5\\times 5$` mask). The overall distance is calculated as a sum of these basic distances. Since the distance function should be symmetric, all of the horizontal and vertical shifts must have the same cost (denoted as a ), all the diagonal shifts must have the same cost (denoted as `b`), and all knight's moves must have the same cost (denoted as `c`). For the [DIST_C](#d7/d1b/group__imgproc__misc_1ggaa2bfbebbc5c320526897996aafa1d8eba507b16eb5ef95ea784ca1b3cb7b0d7ee}) and [DIST_L1](#d7/d1b/group__imgproc__misc_1ggaa2bfbebbc5c320526897996aafa1d8ebae5b2dfaf2ba5024d7ce47885001fad86}) types, the distance is calculated precisely, whereas for [DIST_L2](#d7/d1b/group__imgproc__misc_1ggaa2bfbebbc5c320526897996aafa1d8ebaff0d1f5be0fc152a56a9b9716d158b96}) (Euclidean distance) the distance can be calculated only with a relative error (a `$5\\times 5$` mask gives more accurate results). For `a`,`b`, and `c`, OpenCV uses the values suggested in the original paper:
 * 
 * DIST_L1: `a = 1, b = 2`
 * DIST_L2:
 * 
 ...
 * DIST_C: `a = 1, b = 1`
 * 
 * Typically, for a fast, coarse distance estimation [DIST_L2](#d7/d1b/group__imgproc__misc_1ggaa2bfbebbc5c320526897996aafa1d8ebaff0d1f5be0fc152a56a9b9716d158b96}), a `$3\\times 3$` mask is used. For a more accurate distance estimation [DIST_L2](#d7/d1b/group__imgproc__misc_1ggaa2bfbebbc5c320526897996aafa1d8ebaff0d1f5be0fc152a56a9b9716d158b96}), a `$5\\times 5$` mask or the precise algorithm is used. Note that both the precise and the approximate algorithms are linear on the number of pixels.
 * 
 * This variant of the function does not only compute the minimum distance for each pixel `$(x, y)$` but also identifies the nearest connected component consisting of zero pixels (labelType==[DIST_LABEL_CCOMP](#d7/d1b/group__imgproc__misc_1gga3fe343d63844c40318ee627bd1c1c42fa631de3e838ee72d6a9d991b8fbce4c1d})) or the nearest zero pixel (labelType==[DIST_LABEL_PIXEL](#d7/d1b/group__imgproc__misc_1gga3fe343d63844c40318ee627bd1c1c42fa5d291835de98b72caa12a9947c2cd92a})). Index of the component/pixel is stored in `labels(x, y)`. When labelType==[DIST_LABEL_CCOMP](#d7/d1b/group__imgproc__misc_1gga3fe343d63844c40318ee627bd1c1c42fa631de3e838ee72d6a9d991b8fbce4c1d}), the function automatically finds connected components of zero pixels in the input image and marks them with distinct labels. When labelType==[DIST_LABEL_CCOMP](#d7/d1b/group__imgproc__misc_1gga3fe343d63844c40318ee627bd1c1c42fa631de3e838ee72d6a9d991b8fbce4c1d}), the function scans through the input image and marks all the zero pixels with distinct labels.
 * 
 * In this mode, the complexity is still linear. That is, the function provides a very fast way to compute the Voronoi diagram for a binary image. Currently, the second variant can use only the approximate distance transform algorithm, i.e. maskSize=[DIST_MASK_PRECISE](#d7/d1b/group__imgproc__misc_1ggaaa68392323ccf7fad87570e41259b497a71e13e06c1d12eabd2acd2669f94f9ca}) is not supported yet.
 * 
 ...
 * @param sqsum integral image for squared pixel values; it is $(W+1)\times (H+1)$, double-precision floating-point (64f) array.
 * @param tilted integral for the image rotated by 45 degrees; it is $(W+1)\times (H+1)$ array with the same data type as sum.
 ...
 * 
 * Also, the special values [THRESH_OTSU](#d7/d1b/group__imgproc__misc_1ggaa9e58d2860d4afa658ef70a9b1115576a95251923e8e22f368ffa86ba8bce87ff}) or [THRESH_TRIANGLE](#d7/d1b/group__imgproc__misc_1ggaa9e58d2860d4afa658ef70a9b1115576a22ffcf680811aed95be6c7f5cd809621}) may be combined with one of the above values. In these cases, the function determines the optimal threshold value using the Otsu's or Triangle algorithm and uses it instead of the specified thresh.
 * 
 ...
 * 
 * [adaptiveThreshold](#d7/d1b/group__imgproc__misc_1ga72b913f352e4a1b1b397736707afcde3}), [findContours](#d3/dc0/group__imgproc__shape_1gadf1ad6a0b82947fa1fe3c3d497f260e0}), [compare](#d2/de8/group__core__array_1ga303cfb72acf8cbb36d884650c09a3a97}), [min](#d7/dcc/group__core__utils__softfloat_1gac48df53b8fd34b87e7b121fa8fd4c379}), [max](#d7/dcc/group__core__utils__softfloat_1ga78f988f6cfa6223610298cbd4f86ec66})
 * 
 ...
 * 
 * Before passing the image to the function, you have to roughly outline the desired regions in the image markers with positive (>0) indices. So, every region is represented as one or more connected components with the pixel values 1, 2, 3, and so on. Such markers can be retrieved from a binary mask using [findContours](#d3/dc0/group__imgproc__shape_1gadf1ad6a0b82947fa1fe3c3d497f260e0}) and [drawContours](#d6/d6e/group__imgproc__draw_1ga746c0625f1781f1ffc9056259103edbc}) (see the watershed.cpp demo). The markers are "seeds" of the future image regions. All the other pixels in markers , whose relation to the outlined regions is not known and should be defined by the algorithm, should be set to 0's. In the function output, each pixel in markers is set to a value of the "seed" components or to -1 at boundaries between the regions.
 * 
