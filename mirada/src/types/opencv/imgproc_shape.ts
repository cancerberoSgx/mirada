...
/**
 * The function [cv::approxPolyDP](#d3/dc0/group__imgproc__shape_1ga0012a5fdaea70b8a9970165d98722b4c}) approximates a curve or a polygon with another curve/polygon with less vertices so that the distance between them is less or equal to the specified precision. It uses the Douglas-Peucker algorithm
 * 
...
/**
 * The function finds the four vertices of a rotated rectangle. This function is useful to draw the rectangle. In C++, instead of using this function, you can directly use [RotatedRect::points](#db/dd6/classcv_1_1RotatedRect_1a69d648b086f26dbce0029facae9bfb2d}) method. Please visit the [tutorial on Creating Bounding rotated boxes and ellipses for contours](#de/d62/tutorial_bounding_rotated_ellipses}) for more information.
 * 
...
/**
 * image with 4 or 8 way connectivity - returns N, the total number of labels [0, N-1] where 0 represents the background label. ltype specifies the output label image type, an important consideration based on the total number of labels or alternatively the total number of pixels in the source image. ccltype specifies the connected components labeling algorithm to use, currently Grana (BBDT) and Wu's (SAUF) algorithms are supported, see the [ConnectedComponentsAlgorithmsTypes](#d3/dc0/group__imgproc__shape_1ga5ed7784614678adccb699c70fb841075}) for details. Note that SAUF algorithm forces a row major ordering of labels while BBDT does not. This function uses parallel version of both Grana and Wu's algorithms if at least one allowed parallel framework is enabled and if the rows of the image are at least twice the number returned by [getNumberOfCPUs](#db/de0/group__core__utils_1gadf09fc982bf4f17bc84bd1abce5d0863}).
 * 
 ...
 * @param image the 8-bit single-channel image to be labeled
 * @param labels destination labeled image
 * @param stats statistics output for each label, including the background label, see below for available statistics. Statistics are accessed via stats(label, COLUMN) where COLUMN is one of ConnectedComponentsTypes. The data type is CV_32S.
...
/**
 * The function compares two shapes. All three implemented methods use the Hu invariants (see [HuMoments](#d3/dc0/group__imgproc__shape_1gab001db45c1f1af6cbdbe64df04c4e944}))
 * 
...
/**
 * The function calculates and returns the minimum-area bounding rectangle (possibly rotated) for a specified point set. Developer should keep in mind that the returned [RotatedRect](#db/dd6/classcv_1_1RotatedRect}) can contain negative indices when data is close to the containing [Mat](#d3/d63/classcv_1_1Mat}) element boundary.
 * 
 ...
 * 
 *  The implementation of the algorithm is based on O'Rourke's ORourke86 and Klee and Laskowski's KleeLaskowski85 papers. O'Rourke provides a `$\\theta(n)$` algorithm for finding the minimal enclosing triangle of a 2D convex polygon with n vertices. Since the [minEnclosingTriangle](#d3/dc0/group__imgproc__shape_1ga1513e72f6bbdfc370563664f71e0542f}) function takes a 2D point set as input an additional preprocessing step of computing the convex hull of the 2D point set is required. The complexity of the [convexHull](#d3/dc0/group__imgproc__shape_1ga014b28e56cb8854c0de4a211cb2be656}) function is `$O(n log(n))$` which is higher than `$\\theta(n)$`. Thus the overall complexity of the function is `$O(n log(n))$`.
 * 
...
/**
 * The function computes moments, up to the 3rd order, of a vector shape or a rasterized shape. The results are returned in the structure [cv::Moments](#d8/d23/classcv_1_1Moments}).
 * 
 * moments.
 * 
 ...
 * 
 * [contourArea](#d3/dc0/group__imgproc__shape_1ga2c759ed9f497d4a618048a2f56dc97f1}), [arcLength](#d3/dc0/group__imgproc__shape_1ga8d26483c636be6b35c3ec6335798a47c})
 * 
 ...
 * 
 * One of [RectanglesIntersectTypes](#d3/dc0/group__imgproc__shape_1gaaf0eb9e10bd5adcbd446cd31fea2db68})
 * 
