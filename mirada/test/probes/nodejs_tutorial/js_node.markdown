...
Being able to execute code in node.js, like this tutorial's, is useful for testing, even if the application is only for browsers. This are some advanteges:

 * Test driven development: It's often preferable by some developers being able to execute tests in a terminal instead of using a browser or any graphical environment. 
 * Use testing frameworks like [](https://jestjs.io), [mocha](), [jasmine](), [ava](https://github.com/avajs), etc. In general they are designed to run in the terminal
 * Easy Continuos integration process: Running tests that don't require a browser or any graphic support makes it easier to use CI tools like [travis-ci](https://travis-ci.org) or [azure](https://dev.azure.com).
